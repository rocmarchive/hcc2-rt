...
// Additional worker slot type which is initialized with the default worker slot size of 4*32 bytes.
struct __kmpc_data_sharing_worker_slot_static {
  __kmpc_data_sharing_slot *Next;
  void *DataEnd;
  char Data[DS_Worker_Warp_Slot_Size];
};
// Additional master slot type which is initialized with the default master slot size of 4 bytes.
...
public:
  ...
  INLINE void InitLevelOneTaskDescr(
...
private:
  ...
  //   1 bit (InParL2+) if this thread has encountered L2 or higher parallel region
...
public:
  ...
  INLINE __kmpc_data_sharing_slot *RootS(int wid) {
    ...
    // Initialize the pointer to the end of the slot given the size of the data section. DataEnd is non-inclusive.
    worker_rootS[wid].DataEnd = &worker_rootS[wid].Data[0] + DS_Worker_Warp_Slot_Size;
    // We currently do not have a next slot.
