...
EXTERN int32_t __kmpc_omp_task(kmp_Indent *loc, uint32_t global_tid,
  ...
  return __kmpc_omp_task_with_deps(loc, global_tid, newKmpTaskDescr, 0, 0,
      0, 0);
}
...
EXTERN int32_t __kmpc_omp_task_with_deps(kmp_Indent *loc, uint32_t global_tid,
  ...
  // XXX:[OMPTARGET.FunctionPtr]: Indirect call by a function pointer causes an issue in amdgcn backend
