...
public:
  ...
  INLINE static void for_static_init(int32_t schedtype, T *plower, T *pupper,
                                     ST *pstride, ST chunk) {
    ...
    PRINT(LD_LOOP, "OMP Thread %d: schedule type %d, chunk size = %lld, mytid "
                   "%d, num tids %d\n",
          GetOmpThreadId(gtid), schedtype, P64(chunk), GetOmpThreadId(gtid),
  ...
  INLINE static int dispatch_next(int32_t *plast, T *plower, T *pupper,
    ...
    kmp_sched_t schedule =
        omptarget_nvptx_threadPrivateContext->ScheduleType(gtid);
    ...
    int finished = DynamicNextChunk(
        cg, omptarget_nvptx_threadPrivateContext->Priv(gtid), myLb, myUb,
        chunkId, omptarget_nvptx_threadPrivateContext->CurrentEvent(gtid),
        omptarget_nvptx_threadPrivateContext->Chunk(gtid),
        omptarget_nvptx_threadPrivateContext->LoopUpperBound(gtid));
...
EXTERN void __kmpc_for_static_init_4(kmp_Indent *loc, int32_t global_tid,
  ...
  omptarget_nvptx_LoopSupport<int32_t, int32_t>::for_static_init(
      schedtype, plower, pupper, pstride, chunk);
}
