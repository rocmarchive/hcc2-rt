...
class omptarget_nvptx_GlobalICV;
...
public:
  // task
  INLINE omptarget_nvptx_TaskDescr *Level1TaskDescr(int gtid) {
    return &levelOneTaskDescr[gtid];
  }
  INLINE void SetTopLevelTaskDescr(int gtid,
                                   omptarget_nvptx_TaskDescr *taskICV) {
    topTaskDescr[gtid] = taskICV;
  }
  INLINE omptarget_nvptx_TaskDescr *GetTopLevelTaskDescr(int gtid);
  // parallel
  INLINE uint16_t &NumThreadsForNextParallel(int gtid) {
    return tnumForNextPar[gtid];
  }
  // sync
  ...
  INLINE Counter &CurrentEvent(int gtid) {
    return currEvent_or_nextLowerBound[gtid];
  }
  INLINE Counter &EventsNumber(int gtid) { return eventsNum_or_stride[gtid]; }
  // state for dispatch with static
  INLINE Counter &NextLowerBound(int gtid) {
    return currEvent_or_nextLowerBound[gtid];
  }
  INLINE Counter &Stride(int gtid) { return eventsNum_or_stride[gtid]; }

  INLINE void SetTeamContext(omptarget_nvptx_TeamDescr *teamContext) {
    omptarget_nvptx_teamContext = teamContext;
  }
  INLINE void SetGlobalICV(omptarget_nvptx_GlobalICV *globalICV) {
    omptarget_nvptx_globalICV = globalICV;
  }
  INLINE omptarget_nvptx_TeamDescr *TeamContext() {
    return omptarget_nvptx_teamContext;
  }
  INLINE omptarget_nvptx_GlobalICV *GlobalICV() {
    return omptarget_nvptx_globalICV;
  }
...
private:
  ...
  // Pointers to other omp data in same instance
  // These are initialized in kernel_init
  omptarget_nvptx_TeamDescr *omptarget_nvptx_teamContext;
  omptarget_nvptx_GlobalICV *omptarget_nvptx_globalICV;
};
...
extern __shared__ omptarget_nvptx_ThreadPrivateContext
