...
EXTERN int __kmpc_kernel_prepare_parallel(int NumThreads, int NumLanes) {
  ...
  ASSERT0(LT_FUSSY, GetThreadIdInBlock() == TEAM_MASTER,
...
EXTERN void __kmpc_kernel_parallel(int numLanes) {
  ...
  workDescr.CounterGroup().Init(
      omptarget_nvptx_threadPrivateContext->Priv(globalThreadId));
...
EXTERN void __kmpc_kernel_end_parallel() {
  ...
  omptarget_nvptx_threadPrivateContext->SetTopLevelTaskDescr(
      globalThreadId, currTaskDescr->GetPrevTaskDescr());
}
...
EXTERN void __kmpc_push_num_threads(kmp_Indent *loc, int32_t gtid,
  ...
  omptarget_nvptx_threadPrivateContext->NumThreadsForNextParallel(gtid) =
      num_threads;
}
