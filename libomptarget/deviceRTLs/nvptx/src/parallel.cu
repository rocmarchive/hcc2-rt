...
EXTERN bool __kmpc_kernel_convergent_simd(void *buffer, bool *IsFinal, int32_t *LaneSource,
  ...
  uint32_t ConvergentMask = __BALLOT_SYNC(0xFFFFFFFF, true);
  ...
  *LaneId = __popc(ConvergentMask & lanemask_lt);
  ...
  int32_t SimdLimitSource = __SHFL_SYNC(0xFFFFFFFF, SimdLimit, *LaneSource);
  ...
  ASSERT(LT_FUSSY, *NumLanes > 0,
       "bad thread request of %d threads", *NumLanes);
  ...
  if (*LaneId < *NumLanes) {
    ...
    omptarget_nvptx_TaskDescr *sourceTaskDescr =
        omptarget_nvptx_threadPrivateContext->GetTopLevelTaskDescr(sourceThreadId);
...
EXTERN bool __kmpc_kernel_convergent_parallel(void *buffer, bool *IsFinal, int32_t *LaneSource) {
  ...
  *IsFinal = __popc(WorkRemaining) == 1;
  ...
  int32_t NumThreadsSource = __SHFL_SYNC(0xFFFFFFFF, NumThreadsClause, *LaneSource);
  ...
  ASSERT(LT_FUSSY, NumThreads > 0,
       "bad thread request of %d threads", NumThreads);
...
EXTERN void __kmpc_kernel_prepare_parallel(void *WorkFn, int16_t IsOMPRuntimeInitialized) {
  ...
  ASSERT0(LT_FUSSY, !currTaskDescr->InParallelRegion(), "cannot be called in a parallel region.");
...
EXTERN void __kmpc_push_simd_limit(kmp_Indent *loc, int32_t tid,
  ...
  tid = GetLogicalThreadIdInBlock();
  omptarget_nvptx_threadPrivateContext->SimdLimitForNextSimd(tid) =
      simd_limit;
}
