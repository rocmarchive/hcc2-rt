...
int main(void) {
  ...
  for (int t = 0; t <= 224; t++) {
    ...
    TEST({
      ...
      _Pragma("omp parallel for schedule(dynamic) lastprivate(q2) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q2[0] = C[i] + D[i];
        A[i] += q2[0];
      }
      _Pragma("omp parallel for schedule(guided) lastprivate(q3) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q3[0] = D[i] + E[i];
        B[i] += q3[0];
      }
      _Pragma("omp parallel for schedule(runtime) lastprivate(q4) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q4[0] = C[i] + D[i];
        A[i] += q4[0];
      }
      _Pragma("omp parallel for schedule(static) lastprivate(q5) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q5[0] = D[i] + E[i];
        B[i] += q5[0];
      }
      _Pragma("omp parallel for schedule(static,1) lastprivate(q6) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q6[0] = C[i] + D[i];
        A[i] += q6[0];
      }
      _Pragma("omp parallel for schedule(static,9) lastprivate(q7) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q7[0] = D[i] + E[i];
        B[i] += q7[0];
      }
      _Pragma("omp parallel for schedule(static,13) lastprivate(q8) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q8[0] = C[i] + D[i];
        A[i] += q8[0];
      }
      _Pragma("omp parallel for schedule(static,30000) lastprivate(q9) if(threads[0] > 1) num_threads(threads[0])")
      for (int i = 0; i < N; i++) {
        q9[0] = D[i] + E[i];
        B[i] += q9[0];
      }
  ...
  //
  // Test: ordered clause on omp parallel for.
  //
  printf("K\n");
  #undef PARALLEL_FOR_CLAUSES
  #define PARALLEL_FOR_CLAUSES ordered
  #include "defines.h"
  ...
  TEST({
    ...
    _Pragma("omp parallel for num_threads(33)")
    for (int i = 0; i < 99; i++) {
      A[i] += i - omp_get_thread_num();
    }
    _Pragma("omp parallel for schedule(auto) num_threads(33)")
    for (int i = 0; i < 99; i++) {
      A[i] += i - omp_get_thread_num();
    } 
    _Pragma("omp parallel for schedule(static,1) num_threads(33)")
    for (int i = 0; i < 99; i++) {
      A[i] += i - omp_get_thread_num();
    }
    double tmp = 0;
